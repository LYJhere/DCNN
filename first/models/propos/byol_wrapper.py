...
class BYOLWrapper(MoCoWrapper):
    ...
    def __init__(self,
        ...
        # self.groco_ins = GroCo().cuda()
        # self.groco_clu = GroCo(topk_neg=num_cluster-1).cuda()
    ...
    def forward_loss(self, im_q, im_k, psedo_labels: torch.Tensor):
        ...
        k_n = self.nn_replacer(k.detach(), update=True)

        # contrastive_loss = - 2 * F.cosine_similarity(self.predictor(noise_q), k). mean()
        contrastive_loss = (- 2 * F.cosine_similarity(self.predictor(noise_q), k).mean()) +(
                    - 2 * F.cosine_similarity(self.predictor(noise_q), k_n).mean())
